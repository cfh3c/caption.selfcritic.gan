...
from misc.rewards import get_self_critical_reward, get_self_critical_reward_forTS
...
def train(opt):
    ...
    while True:
        ...
        if not sc_flag:
            ...
            gen_result_S, sample_logprobs_S = modelS.sample(fc_feats, {'sample_max': 0})
            reward_S = get_self_critical_reward_forTS(modelT, modelS, fc_feats, data, gen_result_S, logger)

            gen_result_T, sample_logprobs_T = modelT.sample(fc_feats, {'sample_max': 0})
            reward_T = get_self_critical_reward_forTS(modelS, modelT, fc_feats, data, gen_result_T, logger)

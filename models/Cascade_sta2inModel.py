...
class CascadeNetModel(nn.Module):
    def __init__(self, opt, model1, model2):
        super(CascadeNetModel, self).__init__()
        self.opt = opt
    ...
    def forward(self, fc_feats, att_feats, seq):
        ...
        p_att_feats = p_att_feats.view(*(att_feats.size()[:-1] + (self.model2.att_hid_size,)))
        ...
        for i in range(seq.size(1) - 1):
            if i >= 1 and self.ss_prob > 0.0:  # otherwiste no need to sample
                ...
                if sample_mask.sum() == 0:
                    ...
                    sample_ind = sample_mask.nonzero().view(-1)
                    it = seq[:, i].data.clone()
                    prob_prev = torch.exp(outputs2[-1].data)  # fetch prev distribution: shape Nx(M+1)
        ...
        outputs2 = torch.cat([_.unsqueeze(1) for _ in outputs2], 1).contiguous()
    ...
    def sample(self, fc_feats, att_feats, opt={}, mode=None):
        ...
        for t in range(self.seq_length + 1):
            if t == 0:  # input <bos>
                ...
                if temperature == 1.0:
                    prob_prev = torch.exp(logprobs2.data).cpu()  # fetch prev distribution: shape Nx(M+1)
                else:
                    # scale logprobs by temperature
                    prob_prev = torch.exp(torch.div(logprobs2.data, temperature)).cpu()
                it2 = torch.multinomial(prob_prev, 1).cuda()
